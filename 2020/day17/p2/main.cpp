...
using coordinate = containers::vec4<int64_t>;
...
using wplane = std::map<int64_t, xplane>;
using space = wplane;

auto operator<<(std::ostream& os, const space& s) -> std::ostream&
{
    for(const auto& [w, wdata] : s)
    {
        for(const auto& [z, zdata] : wdata)
        {
            os << "z=" << z << " w=" << w << "\n";
    ...
    for(int64_t w = -1; w <= 1; ++w)
    {
        for(int64_t z = -1; z <= 1; ++z)
        {
            for(int64_t x = -1; x <= 1; ++ x)
            {
                ...
                {
                    coordinate c_prime{c.x + x, c. y + y, c.z + z, c.w + w};
    ...
    for(const auto& [w, wdata] : s)
    {
        for(const auto& [z, zdata] : wdata)
        {
            for(const auto& [x, xdata] : zdata)
            {
                for(const auto& [y, state] : xdata)
                {
                    for(const auto& c : generate_neighbors(coordinate{x, y, z, w}))
                    {
                        s1[c.w][c.z][c.x][c.y];
                    }
                }
        ...
        for(const auto& [z, zdata] : wdata)
        {
            for(const auto& [x, xdata] : zdata)
            {
                for(const auto& [y, state] : xdata)
                {
                    ...
                    for(const auto& c : generate_neighbors(coordinate{x, y, z, w}))
                    {
                        auto w_iter = s.find(c.w);
                        if(w_iter != s.end())
                        {
                            const auto& [w_prime, wdata_prime] = *w_iter;

                            auto z_iter = wdata_prime.find(c.z);
                            if(z_iter != wdata_prime.end())
                            {
                    ...
                    std::cout << "[" << w << ", " << z << ", " << x << ", " << y << "] is " << to_string(state) << " and has " << active_neighbors << " ";
                    ...
                    switch(state)
                    {
                        case state::active:
                            ...
                            {
                                next[w][z][x][y] = state::active;
                                std::cout << " remaining active\n";
                            }
                            else
                            {
                                next[w][z][x][y] = state::inactive;
...
int main(int argc, char* argv[])
{
    ...
    for(const auto& line : chain::str::split(contents, '\n'))
    {
        ...
        {
            s[0][0][x][y] = (c == '#') ? state::active : state::inactive;
            ++y;
        }
        ++x;
    }
