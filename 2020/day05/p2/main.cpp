...
struct boarding_pass
{
    ...
    auto operator<=>(const boarding_pass& other) const -> std::strong_ordering
    {
...
int main(int argc, char* argv[])
{
    ...
    std::set<boarding_pass> boarding_passes{};
    ...
    for(const auto& line : chain::str::split(contents, '\n'))
    {
        ...
        boarding_passes.insert(boarding_pass{row_data, col_data});
    }

    uint64_t previous = (*boarding_passes.begin()).seat_id() - 1;
    for(const auto& bp : boarding_passes)
    {
        if(previous != bp.seat_id() - 1)
        {
            std::cout << "My seat ID is " << (bp.seat_id() - 1) << ".\n";
            break;
        }
        previous = bp.seat_id();
    }
