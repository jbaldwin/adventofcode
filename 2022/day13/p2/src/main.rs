...
fn compare_packets(p1: &Value, p2: &Value) -> Result {
    match p1 {
        Value::List(l1) => {
            match p2 {
                Value::List(l2) => {
...
fn main() {
    ...
    for packet in lines.iter() {
        if packet.len() == 0 {
            continue;
        }

        packets.push((parse_packet(packet), packet.clone()));
    }
    // Add in the divider packets.
    packets.push((parse_packet("[[2]]"), "[[2]]".to_string()));
    packets.push((parse_packet("[[6]]"), "[[6]]".to_string()));

    packets.sort_by(|p1, p2| {
        match compare_packets(&p1.0, &p2.0) {
    ...
    for (i, packet) in packets.iter().enumerate() {
        if packet.1.as_str() == "[[2]]" {
            indices.push(i + 1);
        }
        else if packet.1.as_str() == "[[6]]" {
            indices.push(i + 1);
        }
    }

    println!("{}", indices.iter().fold(1, |a, b| a * b));
