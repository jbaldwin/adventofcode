...
fn main() {
    ...
    print_min_x -= 1;
    max_x += 1; // add some buffer to this side
    max_y += 1; // Anything that drops below max_y is gone forever
    ...
    'outer: loop {
        ...
        loop {
            if cave[(sand.y + 1) as usize][sand.x as usize] == '.' {
                ...
                if sand.y as usize >= max_y {
                    if infinite >= 2 { // 2 seems to work
                        break 'outer;
                    }
    ...
    for y in min_x..=max_y {
        for x in print_min_x..=max_x {
            if y as i64 == START.y && x as i64 == START.x {
                print!("+");
            }
            else {
                print!("{}", cave[y][x]);
