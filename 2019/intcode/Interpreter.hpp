...
public:
    ...
    {
        /// The interpreter requires input to continue executing.
        INPUT_REQUESTED,
        /// The interpreter has output ready to be processed.
        OUTPUT_READY,
        /// The interpreter has finished executing this program.
        COMPLETED
    };
    ...
    /// Loads a set of instructions from a file.
    /// \param instructions_file The file to load instructions from.
    ...
    /// Begins an interpreter with the given set of instructions.
    /// \param instructions Alread loaded instruction set.
    explicit Interpreter(
    ...
    /// Gets the current memory value at the given \c addr.
    /// \param addr The address to retrieve.
    /// \return The value stored at \c addr.
    auto Memory(
        size_t addr
    ) const -> int64_t
    {
        if(auto found = m_memory.find(addr); found != m_memory.end())
        {
            return found->second;
        }
        return 0;
    ...
    /// Sets the memory \c addr to \c value.
    /// \param addr The address to store \c value at.
    /// \param value The value to store at \c addr.
    auto Memory(
        size_t addr,
        int64_t value
    ) -> void
...
private:
    ...
    /// It is important to note that std::map is current used to easily
    /// expand the memory space without requiring bounds checks.  
    /// std::vector<int64_t> would be far more performant but requires bounds
    /// checks and re-allocations complicating several functions.  std::map
    /// trades performance for simplicity in this regard.
