...
public:
    ...
    Interpreter(
        ...
        :   m_memory(std::move(instructions))
    {
        if(m_memory.empty())
        {
...
struct Square
{
    PaintColorEnum color{PaintColorEnum::BLACK};
    bool painted{false};
};
...
int main(int argc, char* argv[])
{
    ...
    std::map<int64_t, std::map<int64_t, Square>> hull_grid{};
    ...
    while(!done)
    {
        ...
        switch(status)
        {
            ...
            {
                auto color = hull_grid[x][y].color;
            ...
            case Interpreter::ExecuteStatusEnum::OUTPUT_READY:
            {
                ...
                if(hull_grid[x][y].color != color)
                {
                    hull_grid[x][y].color = color;
                    hull_grid[x][y].painted = true;
                }
    ...
    int64_t total_painted{0};
    for(const auto& [x, row] : hull_grid)
    {
        for(const auto& [y, square] : row)
        {
            if(square.painted)
            {
                ++total_painted;
