    ...
    for next in connected.iter() {
        ...
        if first_letter.to_ascii_lowercase() == first_letter {
            if visited.contains(next) {
                if !twice_used && next != "start" {
                    pathfind(caves, next.clone(), visited.clone(), paths, true);
                }
                continue;
            }
        }

        pathfind(caves, next.clone(), visited.clone(), paths, twice_used);
    }
}
...
fn main() {
    ...
    pathfind(&caves, "start".to_string(), Vec::new(), &mut paths, false);
